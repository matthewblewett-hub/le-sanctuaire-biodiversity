...
let DATA = FALLBACK_DATA.map(item => ({...item, ...ecologyFor(item.scientific,item.family,item.iconic||'Plantae',item.rank||'species'), ...(PLANTZAFRICA_ENRICHMENT[item.scientific] || {}), nameEtymology:ETYMOLOGY_ENRICHMENT.etymologyFor(item.scientific,item.common)})).sort(sortSpecies);
let group = 'All';
let filter = 'all';
let season = 'all';
...
function aggregate(observations) {
  ...
  return [...buckets.values()].map(records => {
    ...
    const baseMeta = ENRICHMENT[scientific] || {};
    const family = baseMeta.family || familyOf(latestObs);
    const meta = {...baseMeta, ...ecologyFor(scientific,family,taxon.iconic_taxon_name,taxon.rank), ...(PLANTZAFRICA_ENRICHMENT[scientific] || {})};
    const common = meta.common || taxon.preferred_common_name || latestObs.species_guess || scientific;
    const nameEtymology = ETYMOLOGY_ENRICHMENT.etymologyFor(scientific,common);
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=17',{updateViaCache:'none'}).then(reg=>reg.update()).catch(()=>{}));
