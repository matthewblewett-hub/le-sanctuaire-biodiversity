...
let DATA = FALLBACK_DATA.map(item => ({...item, ...ecologyFor(item.scientific,item.family,item.iconic||'Plantae',item.rank||'species'), ...(PLANTZAFRICA_ENRICHMENT[item.scientific] || {})})).sort(sortSpecies);
let group = 'All';
let filter = 'all';
let season = 'all';
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=16',{updateViaCache:'none'}).then(reg=>reg.update()).catch(()=>{}));
