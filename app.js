...
let DATA = FALLBACK_DATA.map(item => ({...item, ...ecologyFor(item.scientific,item.family,item.iconic||'Plantae',item.rank||'species')})).sort(sortSpecies);
let group = 'All';
let filter = 'all';
let season = 'all';
...
function aggregate(observations) {
  ...
  return [...buckets.values()].map(records => {
    ...
    const meta = {...baseMeta, ...ecologyFor(scientific,family,taxon.iconic_taxon_name,taxon.rank)};
...
if('serviceWorker' in navigator) window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=15',{updateViaCache:'none'}).then(reg=>reg.update()).catch(()=>{}));
